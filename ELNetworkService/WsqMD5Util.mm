...
+ (NSString*)getmd5WithString:(NSString *)string
{
    ...
    //截取字符串8-24；
    NSRange range={8,16};
    NSString* result= [outPutStr substringWithRange:range];
    //倒序
    NSMutableString* revseStr=[[[NSMutableString alloc] init] autorelease];
    for (int i=result.length-1;i>=0 ;i--) {
        [revseStr appendFormat:@"%c",[result characterAtIndex:i]];
        
    }
    return [revseStr lowercaseString];
}
